...
class Process_data(object):
    ...
    def get_referenced_place_num(self, paper_id):
        ...
        for item in nodes:
            ...
            if fuzz.partial_ratio('abstract', found_text) >= 65:
                place_num[0] = 1
            elif fuzz.partial_ratio('introduction', found_text) >= 65:
                place_num[1] = 1
            elif fuzz.partial_ratio('related work', found_text) >= 65:
                place_num[2] = 1
            elif fuzz.partial_ratio('method', found_text) >= 65:
                place_num[3] = 1
            elif fuzz.partial_ratio('result', found_text) >= 65 or fuzz.partial_ratio('experiment', found_text) >= 65:
                place_num[5] = 1
            else:
                place_num[6] = 1
...
def extract_paper_info_from_dblp():
    ...
    papers_valid = utils.load_json(data_dir, "paper_source_trace_test_wo_ans.json")

    paper_dict_open = {}
    dblp_fname = "DBLP-Citation-network-V15.json"
    with open(join(data_dir, dblp_fname), "r", encoding="utf-8") as myFile:
        for i, line in tqdm(enumerate(myFile), 
                            total=sum(1 for _ in open(join(data_dir, dblp_fname))),
                            ncols=100, desc="Reading papers"
                           ):
            if len(line) <= 2:
                continue
            if i % 10000 == 0: 
...
def extract_valid_features():
    ...
    out_dir = join(settings.OUT_DIR, "kddcup", "rf")
    os.makedirs(out_dir, exist_ok=True)
    utils.dump_json(total_data_dic, out_dir, "test_data.json")
